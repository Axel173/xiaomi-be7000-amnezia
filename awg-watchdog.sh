...
if [ "$age" -ge "$HS_DEAD" ]; then
    ...
    if [ "$cur" != "FAILOPEN" ]; then
        # --- переход: VPS только что умер ---
        if [ "$mode" != "off" ] && [ "$nbk" -ge 1 ] && [ -x "$SWITCH_VPN" ]; then
            # есть резервы и failover включён → перебор (письма шлёт switch-vpn)
            log "VPS МЁРТВ (handshake ${age}с) → failover (режим=$mode, резервов=$nbk)"
            run_failover
        else
            # режим off, нет резервов или нет switch-vpn — классический fail-open
...
страну: vpn-toggle меню → 9 (Сменить страну)."
        ...
        # --- уже FAILOPEN: периодически пробуем перебор резервов заново ---
        # (вдруг резерв, лежавший в момент падения, уже ожил). Троттл FAILOVER_RETRY.
        if [ "$mode" != "off" ] && [ "$nbk" -ge 1 ] && [ -x "$SWITCH_VPN" ] \
           && [ "$(stamp_age "$FAILOVER_STAMP")" -ge "$FAILOVER_RETRY" ]; then
            log "VPS всё ещё мёртв (${age}с), FAILOPEN → повторный перебор резервов"
            run_failover
        else
