...
apply_routing() {
    [ -x "$AWG_DIR/mark-core.sh" ] && "$AWG_DIR/mark-core.sh" >/dev/null 2>&1
    if [ -x "$AWG_DIR/transport-awg.sh" ]; then
        "$AWG_DIR/transport-awg.sh" up >/dev/null 2>&1
    elif [ -x "$AWG_DIR/split-route.sh" ]; then
...
switch_to() {
    ...
    if ! awg_installed; then
        ...
        printf "Чтобы получить AmneziaWG: переустановите с ПК (be7000.bat -> Установка),\n"
